...
export function ContainerScroll({ titleComponent, children, className }: Readonly<ContainerScrollProps>) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn("h-[64rem] md:h-[80rem] flex items-center justify-center relative p-2 md:p-20", className)}
    >
...
function Card({
  ...
  return (
    <motion.div
      ...
      className="max-w-5xl -mt-12 mx-auto h-[30rem] md:h-[40rem] w-full border-4 border-brand-ink/20 p-2 md:p-6 bg-[#222222] rounded-[30px] shadow-2xl"
    >
      <div className="h-full w-full overflow-hidden rounded-2xl bg-brand-surface md:rounded-2xl">{children}</div>
    </motion.div>
  );
}
