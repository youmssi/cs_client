...
export function Footer({ footer }: Readonly<FooterProps>) {
  const brandName = footer.logo?.company ?? 'Brillance';
  const logoUrl = footer.logo?.image?.url ? getStrapiMediaUrl(footer.logo.image.url) : null;

  // Map Strapi groups to the Brillance 3-column footer layout
  const productLinks = footer.internal_links ?? [];
  const companyLinks = footer.policy_links ?? [];
  const resourceLinks = footer.social_media_links ?? [];

  // Social icons: try to discover from all link groups (common in CMS setups)
  const allLinks = [...productLinks, ...companyLinks, ...resourceLinks];
  const socialLinks = allLinks.filter((l) => {
    const t = (l.text ?? '').toLowerCase();
    return t.includes('twitter') || t === 'x' || t.includes('linkedin') || t.includes('github');
  });
        ...
        <div className="h-auto p-4 md:p-8 flex flex-col justify-start items-start gap-8">
          ...
          <div className="self-stretch flex justify-start items-center gap-3">
            {logoUrl ? (
              ...
              <img src={logoUrl} alt={brandName} className="w-6 h-6 rounded" />
            ) : null}
          ...
          <div className="flex justify-start items-start gap-4">
            {(socialLinks.length ? socialLinks : [{ id: 0, text: 'X', URL: '#', target: '_self' } as LinkType])
              .slice(0, 3)
              .map((link) => (
                <Link
                  key={link.id}
